...
gemma3n = [
        {
            "model": "lmstudio-community/google/gemma-3n-e4b", #change this to point to a new model
            'api_key': 'any string here is fine',
            'api_type': 'openai',
            'base_url': "http://127.0.0.1:1234",
            "cache_seed": random.randint(0, 100000),
...
openai_oss = [
        {
            "model": "openai/gpt-oss-20b", #change this to point to a new model
            'api_key': 'any string here is fine',
            'api_type': 'openai',
            'base_url': "http://127.0.0.1:1234",
            "cache_seed": random.randint(0, 100000),
...
def api_mode (mode, model=None):
    if mode == "local":
        ...
        if model == "gemma3n":
            client = local_client
            completion_model = gemma3n[0]['model']
            embedding_model = local_embedding_model
        if model == None:
...
client, completion_model, embedding_model = api_mode(mode, model="gemma3n")